...
const transformWeeklyData = (rawData, periodKey, componentName) => {
  ...
  // Filter out current incomplete week for all weekly periods
  const currentWeekStart = getCurrentWeekStart()
  const currentWeekKey = currentWeekStart.toISOString().slice(0, 10)
  
  const completedWeeks = allWeekStarts.filter(weekStart => weekStart !== currentWeekKey)
  
  // Take exactly the number of weeks expected for this period
  const weeksToUse = completedWeeks.slice(-config.weeks)
  
  // Generate chart data ensuring continuous sequence
  ...
  logger.log(`${componentName}: ✓ Transformed weekly data for ${periodKey}: ${aggregatedData.length} weeks`)
  return aggregatedData
}
